...
# deixando o Plannotator cair no default. Isso é cosmético (um selo na UI):
# jamais deixe a detecção bloquear o portão.
...
detect_harness() {
  ...
  # Subimos 3 níveis de PPID porque a ferramenta bash do harness pode interpor
  # um shell entre o agente e este script.
...
# ---------------------------------------------------------------------------
# Parse do envelope --json
# ---------------------------------------------------------------------------
# Contrato do Plannotator (verificado no binário 0.19.17):
#   {"decision":"approved"}
#   {"decision":"dismissed"}
#   {"decision":"annotated","feedback":"<markdown>"}
# O campo feedback traz \n e " escapados — extrair com sed seria errado, então
# exigimos jq OU python3. Nenhum dos dois: exit 2 com mensagem acionável.
JSON_TOOL=""
pick_json_tool() {
  ...
  err "plan-approval.sh: preciso de jq OU python3 no PATH para ler o envelope --json"
