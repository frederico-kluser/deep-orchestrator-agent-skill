...
5. **Purge final:** em toda execução (com ou sem `wt=`), o fim roda `do-wt.sh purge`, que fecha TODAS as linhas do owned.tsv — inclusive worktrees kind=test/validation, snapshots `int-ondaN-*` e linhas BLOCKED/ORPHANED — removendo cada worktree e arquivando cada branch em `refs/do-archive/$RUN_ID/` antes de apagá-lo. Zero worktrees/branches de sub-agente sobrevivem; a única exceção é a própria worktree wt-root, persistente por design.
